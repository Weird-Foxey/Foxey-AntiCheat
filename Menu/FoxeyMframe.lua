FoxeyM = { }
FoxeyM.__index = FoxeyM

-- Deprecated
FoxeyM.debug = true
function FoxeyM.SetDebugEnabled(enabled)
end
function FoxeyM.IsDebugEnabled()
	return false
end
---
...
function FoxeyM.CreateMenu(id, title, subTitle, style)
	-- Default settings
	local menu = { }
...
function FoxeyM.CreateSubMenu(id, parent, subTitle, style)
	local parentMenu = menus[parent]
	if not parentMenu then
		return
	end

	FoxeyM.CreateMenu(id, parentMenu.title, subTitle and string.upper(subTitle) or parentMenu.subTitle)
...
function FoxeyM.CurrentMenu()
	return currentMenu and currentMenu.id or nil
end

function FoxeyM.OpenMenu(id)
...
function FoxeyM.IsMenuOpened(id)
	return currentMenu and currentMenu.id == id
end
FoxeyM.Begin = FoxeyM.IsMenuOpened

function FoxeyM.IsAnyMenuOpened()
	return currentMenu ~= nil
end

function FoxeyM.IsMenuAboutToBeClosed()
	return false
end

function FoxeyM.CloseMenu()
	if currentMenu then
...
function FoxeyM.ToolTip(text, width, flipHorizontal)
	if not currentMenu then
		return
	end
...
function FoxeyM.Button(text, subText)
	if not currentMenu then
		return
	end
...
function FoxeyM.SpriteButton(text, dict, name, r, g, b, a)
	if not currentMenu then
		return
	end

	local pressed = FoxeyM.Button(text)
...
function FoxeyM.InputButton(text, windowTitleEntry, defaultText, maxLength, subText)
	if not currentMenu then
		return
	end

	local pressed = FoxeyM.Button(text, subText)
	local inputText = nil
...
function FoxeyM.MenuButton(text, id, subText)
	if not currentMenu then
		return
	end
...
function FoxeyM.CheckBox(text, checked, callback)
	if not currentMenu then
		return
	end
	...
	local pressed = FoxeyM.SpriteButton(text, 'commonmenu', name)
...
function FoxeyM.ComboBox(text, items, currentIndex, selectedIndex, callback)
	if not currentMenu then
		return
	end
	...
	local pressed = FoxeyM.Button(text, selectedItem)
...
FoxeyM.End = FoxeyM.Display

function FoxeyM.CurrentOption()
...
function FoxeyM.IsItemSelected()
	return currentKey == keys.select and FoxeyM.IsItemHovered()
end

function FoxeyM.SetTitle(id, title)
	setMenuProperty(id, 'title', title)
end
FoxeyM.SetMenuTitle = FoxeyM.SetTitle

function FoxeyM.SetSubTitle(id, text)
	setMenuProperty(id, 'subTitle', string.upper(text))
end
FoxeyM.SetMenuSubTitle = FoxeyM.SetSubTitle

function FoxeyM.SetMenuStyle(id, style)
	setMenuProperty(id, 'style', style)
end

function FoxeyM.SetMenuX(id, x)
	setStyleProperty(id, 'x', x)
end

function FoxeyM.SetMenuY(id, y)
	setStyleProperty(id, 'y', y)
end

function FoxeyM.SetMenuWidth(id, width)
	setStyleProperty(id, 'width', width)
end

function FoxeyM.SetMenuMaxOptionCountOnScreen(id, count)
	setStyleProperty(id, 'maxOptionCountOnScreen', count)
end

function FoxeyM.SetTitleColor(id, r, g, b, a)
	setStyleProperty(id, 'titleColor', { r, g, b, a })
end
FoxeyM.SetMenuTitleColor = FoxeyM.SetTitleColor

function FoxeyM.SetMenuSubTitleColor(id, r, g, b, a)
	setStyleProperty(id, 'subTitleColor', { r, g, b, a })
end

function FoxeyM.SetTitleBackgroundColor(id, r, g, b, a)
	setStyleProperty(id, 'titleBackgroundColor', { r, g, b, a })
end
FoxeyM.SetMenuTitleBackgroundColor = FoxeyM.SetTitleBackgroundColor

function FoxeyM.SetTitleBackgroundSprite(id, dict, name)
	RequestStreamedTextureDict(dict)
	setStyleProperty(id, 'titleBackgroundSprite', { dict = dict, name = name })
end
FoxeyM.SetMenuTitleBackgroundSprite = FoxeyM.SetTitleBackgroundSprite

function FoxeyM.SetMenuBackgroundColor(id, r, g, b, a)
	setStyleProperty(id, 'backgroundColor', { r, g, b, a })
end

function FoxeyM.SetMenuTextColor(id, r, g, b, a)
	setStyleProperty(id, 'textColor', { r, g, b, a })
end

function FoxeyM.SetMenuSubTextColor(id, r, g, b, a)
	setStyleProperty(id, 'subTextColor', { r, g, b, a })
end

function FoxeyM.SetMenuFocusColor(id, r, g, b, a)
	setStyleProperty(id, 'focusColor', { r, g, b, a })
end

function FoxeyM.SetMenuFocusTextColor(id, r, g, b, a)
	setStyleProperty(id, 'focusTextColor', { r, g, b, a })
end

function FoxeyM.SetMenuButtonPressedSound(id, name, set)
